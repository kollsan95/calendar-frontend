...
const Modal = {
    showCreate(day, month, year, selectedRange = null) {
        ...
        modal.innerHTML = `
            ...
            <div class="form-group"><label>Время</label><div style="display:flex;gap:12px;align-items:center;">
                ...
                <select id="modalEndHour" style="flex:1;padding:10px;border:1.5px solid #E0F2F1;border-radius:12px;font-size:16px;">${this._generateHourOptions(10, 21)}</select>
            </div></div>
        ...
        document.getElementById('modalSave').addEventListener('click', async () => { await this._saveRecord(date); });
    },

    close() { const el = document.getElementById('modalOverlay'); if (el) el.remove(); },
    ...
    async _saveRecord(date) {
        ...
        if (startHour >= endHour) { alert('Время начала должно быть меньше времени окончания'); return; }

        const isAdmin = Auth.isAdmin();
        const currentUser = Auth.getUser();
        if (isAdmin && username !== currentUser.username) {
            if (!insta || !phone) { alert('Для записи другого пользователя необходимо указать Instagram и телефон'); return; }
        }

        try {
            const response = await API.saveRecord({ date, startHour, endHour, username, serviceType, insta, phone });
